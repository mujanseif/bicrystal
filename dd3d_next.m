...
%     string1=strcat('.\output\gputest\','output');
...
%         view(viewangle);
        view([0.3 -1 0.4])
        saveas(gcf,['./output/gputest/',fname,'-',num2str(curstep),'.png'])
        saveas(gcf,['./output/gputest/',fname,'-',num2str(curstep)])
...
%             dotarea = norm(ep_inc(3,3))/(dt); % strain rate
    ...
    if mod(curstep, 5)==0
        ...
        save(sprintf('./output/gputest/%s_%d',fname,curstep),'-regexp','^(?!(K|kg|Kred|Lred|Ured)$).')
    end
%    save restart;

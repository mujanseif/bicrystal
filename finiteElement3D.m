...
d = dz/mz; % element depth, in z direction 

mx = round(mz*dx/dz); % elements in z direction
mx=max(mx,1);
w = dx/mx; % elements width


my = round(mz*dy/dz); % # elements in y direction
my=max(my,1);
h=dy/my; % element height


mel = mx*my*mz;  % element number 
mno=(mx+1)*(my+1)*(mz+1); % node number
...
% tic
disp('global stiffness matrix assembly new (fast) method...')
...
if loading == 0 % cantilever bending
...
elseif loading== 1 % compression 
    % define empty sets (not used but allows same code as for bending)
    % priority for inclusion of shared nodes is in order below
    %S = [node #, nodal Area, outward normal]
    Smixed = []; % top right edge (u3=U) first priority
    
    Stop =  zeros((mx+1)*(my+1),5);  % 1st priority
    Sbot =  zeros((mx+1)*(my+1),5);
    
    Sleft = zeros((my+1)*(mz-1),5);  % 2nd priority
    Sright = zeros((my+1)*(mz-1),5); 
    
    Sfront = zeros((mz-1)*(mx-1),5); % 3rd priority
    Sback = zeros((mz-1)*(mx-1),5);
...
if loading == 0 % end loaded cantilever
...
elseif loading == 1 % compression
    ...
    gammat=[Stop]; % t=0         
    gammau=[Sleft;Sright;Sfront;Sback;Sbot]; %  set containing fixed nodes u=0    

function [vn,links,fn] = mobbcc_bb1b(fseg,rn,links,connectivity,nodelist,conlist)
%mobility law function (model: BCC0)
global Bscrew Bedge Beclimb do_inclusion do_twosource
nodelist1=nodelist;
conlist1 =conlist;
...
vn = zeros(L1,3);
fn=zeros(L1,3);
planenorm=zeros(size(links,1),3);
...
%                     Bclimb=sqrt( (Beclimb^2 ) * sinth2 + ( Bscrew^2 ) * costh2);
                 ...
                 costh2=(linedir*burgv')^2/(burgv*burgv'); % (lhat.bhat)^2 = cos^2(theta)                                              % calculate how close to screw the link is
                 sinth2=1-costh2;
                 ndir=cross(burgv,linedir)./sqrt((burgv*burgv')*sinth2);
...
if do_inclusion ==1
 %   [vn]=inc_velocity_accelerate(vn,rn,links,connectivity,nodelist1,conlist1 ); % calculate the dislocation glide mobility near inclusions
    [vn]=touchboundary_velocity(vn,rn,links,connectivity,nodelist1,conlist1 );
%     if any(any(isnan(vn)))
%         disp('NaN in vn! See Line 178 in mobbcc_bb1b.m');
%     end
    % else
%     vn=vn;
end
